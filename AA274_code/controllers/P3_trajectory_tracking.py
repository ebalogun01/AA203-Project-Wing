...
class TrajectoryTracker:
    ...
    def compute_control(self, x, y, th, t):
        ...
        ########## Code starts here ##########
        x_dd = xdd_d+self.kpx*(x_d-x)+self.kdx*(xd_d-self.V_prev*np.cos(th))
        y_dd = ydd_d+self.kpy*(y_d-y)+self.kdy*(yd_d-self.V_prev*np.sin(th))

        a = x_dd*np.cos(th)+y_dd*np.sin(th)
        V = a*dt + self.V_prev
        om = -1/V*x_dd*np.sin(th)+1/V*y_dd*np.cos(th)

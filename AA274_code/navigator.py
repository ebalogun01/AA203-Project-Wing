...
class Navigator:
    ...
    def __init__(self):
        ...
        ##################
        try:
            self.food_locations = np.load('food_locations.npy', allow_pickle=True).item()
        except:
            self.food_locations = {'home': (Pose2D(0,0,0), 1.0)}
        self.current_goal = ''
        self.pickup = ''
        ###################
        ...
        objects = ['banana', 'donut', 'bowl', 'cat']
        for obj in objects:
            rospy.Subscriber('/detector/{}'.format(obj), DetectedObject, self.object_detected_callback)
        rospy.Subscriber('/delivery_loc',String,self.delivery_callback)
        #################################################
        print 'Initialization complete, saved locations:', self.food_locations.keys()

        self.teleop_time = rospy.get_rostime()
        rospy.Subscriber('/cmd_vel/teleop_on', String, self.teleop_callback)
    ...
    def object_detected_callback(self, msg):
        """ callback for when the detector has found a food object. Note that
        a distance of 0 can mean that the lidar did not pickup the object at all """
        print 'detected', msg.name, 'with confidence', msg.confidence
        pose = Pose2D(self.x, self.y, self.theta)
        if msg.name not in self.food_locations \
                or msg.confidence > self.food_locations[msg.name][1]:
            self.food_locations[msg.name] = (pose, msg.confidence)
            np.save('food_locations.npy', self.food_locations)
            print 'updated location'

    def delivery_callback(self,msg):
        """ callback to set the goal position to the desired food location"""

        if msg.data != self.current_goal:
            self.current_goal = msg.data
            pose = self.food_locations[self.current_goal][0]
            self.x_g = pose.x
            self.y_g = pose.y
            self.theta_g = pose.theta
            print 'updating goal to', self.current_goal
    ...
    def publish_control(self):
        ...
        if (rospy.get_rostime() - self.teleop_time).to_sec() > 2.0:
            self.nav_vel_pub.publish(cmd_vel)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            self.pickup_pub.publish(self.pickup)

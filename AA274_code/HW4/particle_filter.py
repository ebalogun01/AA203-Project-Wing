...
class ParticleFilter(object):
    ...
    def transition_update(self, u, dt):
        ...
        M = self.xs.shape[0]
        self.xs = self.transition_model(np.tile(u,(M,1)),dt)
...
class MonteCarloLocalization(ParticleFilter):
    ...
    def transition_model(self, us, dt):
        ...
        g = np.zeros((M,3))
        for i in range(M):
            g[i,:] = tb.compute_dynamics(self.xs[i,:],us[i,:],dt,False)
    ...
    def measurement_update(self, z_raw, Q_raw):
        ...
        for m in range(self.M):
            ws[m] = scipy.stats.multivariate_normal.pdf(vs[m,:],mean=np.zeros(Q.shape[0]),cov=Q)
    ...
    def measurement_model(self, z_raw, Q_raw):
        ...
        # TODO: Compute Q.
        if len(Q_raw)>1:
            Q = scipy.linalg.block_diag(Q_raw[0],Q_raw[1])
        else:
            Q = Q_raw[0]
        for k in range(2,len(Q_raw)):
            Q = scipy.linalg.block_diag(Q,Q_raw[k])
    ...
    def compute_innovations(self, z_raw, Q_raw):
        ...
        for m in range(self.M):
            for i in range(I):
                for j in range(J):
                    V[i,j,0] = angle_diff(z_raw[0,i],hs[m,0,j])
                    V[i,j,1] = z_raw[1,i]-hs[m,1,j]
                    d[i,j] = np.matmul(np.transpose(V[i,j,:]),np.matmul(np.linalg.inv(Q_raw[i,:,:]),V[i,j,:]))
    ...
    def compute_predicted_measurements(self):
        ...
        for m in range(self.M):
            for j in range(J):
                h = tb.transform_line_to_scanner_frame(self.map_lines[:,j],self.xs[m],self.tf_base_to_camera,False)

...
class Ekf(object):
    ...
    def measurement_update(self, z_raw, Q_raw):
        ...
        self.Sigma -= np.matmul(K,np.matmul(S,np.transpose(K)))
...
class EkfLocalization(Ekf):
    ...
    def measurement_model(self, z_raw, Q_raw):
        ...
        z = z.flatten()
        if len(Q_list)>1:
            Q = scipy.linalg.block_diag(Q_list[0],Q_list[1])
        else:
            Q = Q_list[0]
        for k in range(2,len(Q_list)):
    ...
    def compute_innovations(self, z_raw, Q_raw):
        ...
        V = np.zeros((I,J,2))
        d = np.zeros((I,J))
        #compute innovations v^{ij} and Mahalanobis distances
        for i in range(I):
            for j in range(J):
                V[i,j,0] = angle_diff(z_raw[0,i],hs[0,j])
                V[i,j,1] = z_raw[1,i]-hs[1,j]
...
class EkfSlam(Ekf):
    ...
    def transition_model(self, u, dt):
        ...
        g[0:3],Gx[0:3,0:3],Gu[0:3,0:2] = tb.compute_dynamics(self.x,u,dt)
    ...
    def compute_predicted_measurements(self):
        ...
        for j in range(J):
            ...
            ########## Code starts here ##########
            # TODO: Compute h, Hx.
            h,Hx[:,0:3] = tb.transform_line_to_scanner_frame(self.x[idx_j:idx_j+2],self.x,self.tf_base_to_camera)
            Hx[0,idx_j] = 1
            Hx[0,idx_j+1] = 0
            Hx[1,idx_j] = -np.sqrt(self.x[0]**2+self.x[1]**2)*np.sin(np.arctan2(self.x[1],self.x[0])-alpha)-np.sqrt(self.tf_base_to_camera[0]**2+self.tf_base_to_camera[1]**2)*np.sin(np.arctan2(self.tf_base_to_camera[1],self.tf_base_to_camera[0])-alpha+self.x[2])
            Hx[1,idx_j+1] = 1

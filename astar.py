...
class AStar(object):
    ...
    def is_free(self, x):
        ...
        return self.occupancy.is_free(x) and in_bounds
        ########## Code ends here ##########
    ...
    def get_neighbors(self, x):
        ...
        neighbors = []
        ########## Code starts here ##########
        x1_neigh = (x[0]-self.resolution,x[0],x[0]+self.resolution)
        x2_neigh = (x[1]-self.resolution,x[1],x[1]+self.resolution)
        for i in x1_neigh:
            for j in x2_neigh:
                neigh = (i,j)
                neigh = self.snap_to_grid(neigh)
                if neigh==x:
                    continue
    ...
    def solve(self):
        ...
        while len(self.open_set)>0:
            ...
            for neigh in self.get_neighbors(x_curr):
                if neigh in self.closed_set:
                    continue
                tent_cost_to_arrive = self.cost_to_arrive[x_curr]+self.distance(x_curr,neigh)
                if neigh not in self.open_set:
                    self.open_set.add(neigh)
                elif tent_cost_to_arrive > self.cost_to_arrive[neigh]:
                    continue
                self.came_from[neigh] = x_curr
                self.cost_to_arrive[neigh] = tent_cost_to_arrive
                self.est_cost_through[neigh] = tent_cost_to_arrive + self.distance(neigh,self.x_goal)
        return False
        ########## Code ends here ##########
